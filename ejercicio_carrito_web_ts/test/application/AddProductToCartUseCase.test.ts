...
describe("Test para AddProductToCartUseCase", () => {
  it("Deberia agregar un producto al carrito con exito", () => {
    const mockRepository = {
      findProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 20)),
      getAllProducts: jest.fn().mockReturnValue([]),
      getProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 20)),
      saveProduct: jest.fn(),
      updateProduct: jest.fn(),
      getUserById: jest.fn().mockReturnValue(new User(123, "Usuario 123", "user@gmail.com")),
      saveCart: jest.fn().mockReturnValue(undefined),
      getCartByUserId: jest.fn().mockReturnValue(undefined),
    };

    const addToCartUseCase = new AddProductToCartUseCase(mockRepository as ProductRepository);
  ...
  it("Deberia lanzar error si el producto no existe", () => {
    ...
    const useCase = new AddProductToCartUseCase(mockRepository as unknown as ProductRepository);

    expect(() => useCase.addToCart(1, 2, 123)).toThrow("Producto no encontrado");
  });

  it("Deberia lanzar error si el usuario no existe", () => {
    const mockRepository = {
      findProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 10)),
      getUserById: jest.fn().mockReturnValue(undefined),
        getProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 10)),
    };
  ...
  it("Deberia lanzar error si la cantidad excede el stock", () => {
    const mockRepository = {
      findProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 5)),
      getUserById: jest.fn().mockReturnValue(new User(123, "Usuario 123", "user@gmail.com")),
      getProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 5)),
    };
  ...
  it("Deberia incrementar la cantidad si el producto ya existe en el carrito", () => {
    ...
    const mockCart = {
      ...
      addProduct: jest.fn(),
      getItemCarts: jest.fn().mockReturnValue([{ getProduct: () => product, getQuantity: () => 2, setQuantity: jest.fn() }]),
    };
  ...
  it("Deberia lanzar error si el producto no tiene stock", () => {
    const mockRepository = {
      findProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 0)),
      getUserById: jest.fn().mockReturnValue(new User(123, "Usuario 123", "user@gmail.com")),
      getProductById: jest.fn().mockReturnValue(new Product(1, "Producto 1", 10.0, 0)),
    };
