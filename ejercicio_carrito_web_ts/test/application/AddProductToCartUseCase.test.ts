...
describe("Test para AddProductToCartUseCase", () => {
  it("Deberia agregar un producto al carrito con exito", () => {
    const mockRepository = {
      findProductById: jest.fn().mockReturnValue({ id: 1, name: "Producto 1", price: 10.0, stock: 20 }),
      getAllProducts: jest.fn().mockReturnValue([]),
      getProductById: jest.fn().mockReturnValue(undefined),
      saveProduct: jest.fn(),
      updateProduct: jest.fn(),
      getUserById: jest.fn().mockReturnValue({ id: 123, name: "Usuario 123" }),
    };
...
it("Deberia lanzar error si el producto no existe", () => {
  const mockRepository = {
    ...
    getUserById: jest.fn().mockReturnValue({ id: 123 }),
...
it("Deberia lanzar error si el usuario no existe", () => {
  const mockRepository = {
    findProductById: jest.fn().mockReturnValue({ id: 1, stock: 10 }),
...
it("Deberia lanzar error si la cantidad es negativa", () => {
  ...
  expect(() => useCase.addToCart(1, -1, 123)).toThrow();
});

it("Deberia lanzar error si la cantidad excede el stock", () => {
  const mockRepository = {
    findProductById: jest.fn().mockReturnValue({ id: 1, stock: 5 }),
    getUserById: jest.fn().mockReturnValue({ id: 123 }),
  };
...
it("Deberia incrementar la cantidad si el producto ya existe en el carrito", () => {
  ...
  const mockRepository = {
    findProductById: jest.fn().mockReturnValue({ id: 1, stock: 10 }),
    getUserById: jest.fn().mockReturnValue({ id: 123, cart: mockCart }),
  };
  ...
  expect(mockCart.addProduct).toHaveBeenCalledWith(1, 3);
});

it("Deberia crear un carrito si el usuario no tiene uno", () => {
  const mockRepository = {
    findProductById: jest.fn().mockReturnValue({ id: 1, stock: 10 }),
    getUserById: jest.fn().mockReturnValue({ id: 123, cart: null }),
    saveCart: jest.fn(),
  };
...
it("Deberia lanzar error si el producto no tiene stock", () => {
  const mockRepository = {
    findProductById: jest.fn().mockReturnValue({ id: 1, stock: 0 }),
    getUserById: jest.fn().mockReturnValue({ id: 123 }),
  };

  const useCase = new AddProductToCartUseCase(mockRepository as unknown as ProductRepository);

  expect(() => useCase.addToCart(1, 1, 123)).toThrow("Sin stock");
});
